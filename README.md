...
DMM英会話のレッスン履歴ページ（`/lesson/`）をスクレイピングし、受講データをローカルに保存・分析する。レッスン中に教材名を自動取得してレコードに追加する機能も備える。外部サービスへのデータ送信は一切行わない。
...
├── background.js          # Service Worker（スクレイピング・データ管理・教材名取得）
...
| 項目 | 方針 |
|------|------|
| ネットワークアクセス | `https://eikaiwa.dmm.com/lesson/*` および `https://api.engoo.com/api/lesson_bookings/*` のみ |
