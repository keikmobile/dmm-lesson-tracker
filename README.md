...
- 受講時間
- レッスンノートへのリンク
- 🎙 ボタン（録音ダウンロード。`lesson_booking_url` があり 2022-03-23 以降のレコードのみ表示）
...
ファイル名は `dmm-lesson-YYYY-MM-DD-HHMM-教材名.webm` 形式。教材名が未取得の場合は `dmm-lesson-YYYY-MM-DD-HHMM.webm`。

> **前提:** 事前に「履歴を取得」を実行して `lesson_booking_url` をレコードに保存しておくこと。2022年3月23日より前のレッスンは録音が存在しないためダウンロードボタンは表示されない。
